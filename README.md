...
!\[Workspace with chart and AI insight](docs/workspace.png)



!\[Dataset profiling](docs/datasets.png)



!\[Query history](docs/history.png)



!\[Safety engine — 14/14 attack tests passing](docs/safety-test.png)
...
```env
   ...
   GROQ\_API\_KEY=your\_groq\_key       # free at console.groq.com
   GEMINI\_API\_KEY=your\_gemini\_key   # free at aistudio.google.com (enables the RAG cache)
   ```
...
as defence in depth.
   ...
   ```bash
docker compose exec backend python app/tests/test\_safety\_engine.py
```
...
* `Show total monthly\_revenue by plan`
* `Show the number of customers per country`
* `Count customers by acquisition channel`
* `Show the trend of monthly\_revenue over signup\_date`
* `Find anomalies in monthly\_revenue`
* `Forecast monthly\_revenue for the next 6 months`
...
backend/
  app/
    ...
    services/     safety\_engine, query\_service, rag\_cache, embeddings,
                  chart\_selector, insight\_engine, analytics\_engine, profiling\_engine
